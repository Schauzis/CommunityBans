/*
 *This entire class file has been commented out (and will soon be removed) because it
 * deals with a messaging and inbox system. I feel that this is completely unneeded in
 * a banning plugin and thus I am phasing it out. In 1 or 2 more commits, I do not expect
 * to even be able to find this file in the source.
...
public class messager extends Thread {
	...
	public messager(CommunityBans p, String action, String playerName,
			String targetName, String messageID, String message) {
		MCBans = p;
			...
			if (response.containsKey("result")) {
				if (response.get("result").equals("n")) {
					MCBans.broadcastPlayer(
							PlayerName,
							ChatColor.DARK_GRAY
									+ MCBans.Language.getFormat("inboxEmpty",
											PlayerName));
					return;
					...
					MCBans.broadcastPlayer(
							...
							ChatColor.DARK_GREEN
									+ MCBans.Language.getFormat("InboxHeader",
											PlayerName));
			...
			if (response.containsKey("result")) {
				if (response.get("result").equals("n")) {
					MCBans.broadcastPlayer(
							PlayerName,
							ChatColor.DARK_GRAY
									+ MCBans.Language.getFormat("messageNone",
											PlayerName));
					return;
				} else if (response.get("result").equals("y")) {
					MCBans.broadcastPlayer(
							PlayerName,
							ChatColor.DARK_GREEN
									+ MCBans.Language.getFormatMessageView(
											"messageView",
											response.get("sender"),
											response.get("date"),
			...
			if (response.containsKey("result")) {
				if (response.get("result").equals("n")) {
					MCBans.broadcastPlayer(
							PlayerName,
							ChatColor.DARK_RED
									+ MCBans.Language.getFormat("messageError",
											PlayerName));
					return;
				} else if (response.get("result").equals("y")) {
					MCBans.broadcastPlayer(PlayerName, ChatColor.DARK_GREEN
							+ MCBans.Language.getFormat("messageSent"));
				}
			}
			break;
		case 3:
			...
			if (response.containsKey("result")) {
				if (response.get("result").equals("a")) {
					MCBans.broadcastPlayer(
							PlayerName,
							ChatColor.DARK_RED
									+ MCBans.Language.getFormat("blockAlready",
											PlayerName));
					return;
				} else if (response.get("result").equals("y")) {
					MCBans.broadcastPlayer(
							PlayerName,
							ChatColor.DARK_GREEN
									+ MCBans.Language.getFormat("blockSuccess",
											TargetName));
				}
			}
			break;
			...
			if (response.containsKey("result")) {
				if (response.get("result").equals("n")) {
					MCBans.broadcastPlayer(PlayerName, ChatColor.DARK_RED
							+ MCBans.Language.getFormat("blockNot", PlayerName));
					return;
				} else if (response.get("result").equals("y")) {
					MCBans.broadcastPlayer(
							PlayerName,
							ChatColor.DARK_GREEN
									+ MCBans.Language.getFormat(
											"unBlockSuccess", TargetName));
				}
			}
			break;
		}
